...
问题卡片使用紧凑的纵向编号选项、分页、自由回答、跳过与发送；收起后仍可重新打开。未回答的分页会先定位，提交期间拦截重复发送，流式更新保留草稿和输入焦点。主聊与侧聊的技能选择列表均在名称下显示来源根目录，完整路径放在悬停提示。历史房间搜索框使用主题边框。

思考和工具过程按活动标识更新原有折叠节点，避免流式输出在鼠标按下与抬起之间替换控件。Claude 自选 MCP 通过本次调用的原生 deniedMcpServers 设置阻止禁用服务初始化；工具调用过滤作为第二层保护。普通聊天不再为了补能力清单而自动扫描连接服务；缓存不可用时保留明确的安全降级提示。此初始化边界通过本机 Claude 的隔离 MCP 进程握手验证，其他 CLI 版本仍应通过相应原生验收。

0.6.3 发布后的源码修复首次纳入本版：费用来源优先级、未知用量保留与失败重试持久化顺序。修改源码不会改变已有 EXE；使用本版安装器或完整解压本版 ZIP 后启动。发布前对源码、包内文件和 GitHub 下载校验值分别验收。

## 0.6.2 变更

明确共享 Skill 的显式文件调用：不要求原生安装，先读取指定的 SKILL.md，以该文件目录解析相对资源，项目操作仍使用本轮工作目录，并遵循原生权限审批。Codex、Claude 已用真实模型读取未安装的工作区外 Skill 和资源完成验证。

修复 Claude 在思考块之后返回单块文字快照时，最终回复被追加两次的问题；依据同一消息的原生块标识合并，保留不同块或不同消息的合法重复内容。

补充子代理卡片到侧边工作台的点击与输出更新验收，以及 [远程访问方案](ARCHITECTURE.md#remote-access-proposal)。远程服务与 Job Object/cgroup 隔离尚未实现。
