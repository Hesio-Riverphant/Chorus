...
Native discovery, output buffers, activity histories and pending RPC requests are bounded. A Stop request cancels capability preparation and active Agent processes. Pending native questions are held in memory and restored to a refreshed renderer; they are not written as application records.
